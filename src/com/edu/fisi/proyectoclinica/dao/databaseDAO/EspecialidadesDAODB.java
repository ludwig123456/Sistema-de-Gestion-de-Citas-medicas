...
public class EspecialidadesDAODB  implements EspecialidadesDAO {
    ...
    public String read(String e) {
        ...
        try {
            ...
            conexion = acceso.getConexion();
            ps = conexion.prepareStatement("SELECT especialidad FROM especialidad WHERE especialidad = ?");
            ps.setString(1, e);
            rs = ps.executeQuery();
    ...
    public List<String> readAll() {
        ...
        try {
            //Obtenemos la conexion
            conexion = acceso.getConexion();
            ps = conexion.prepareStatement("SELECT especialidad FROM especialidades");
            rs = ps.executeQuery();
    ...
    @Override
    public List<Float> readAllP() {
          List<Float> listaHistoriaC = new ArrayList<>();
        Connection conexion = null;
        try {
            //Obtenemos la conexion
            conexion = acceso.getConexion();
            ps = conexion.prepareStatement("SELECT precio FROM especialidades");
            rs = ps.executeQuery();

            while (rs.next()) {

                listaHistoriaC.add(rs.getFloat("precio"));
            }
